...
| Gate evaluation | `causal_edge/validation/metrics.py` | Live failures are T6/T7/T12/T13/T14/T15-Lo/T15-Omega/T15-MaxDD plus `PnL floor`, `Sharpe/Lo`, and conditional `IC`/`IC stab` |
| Result contract | `causal_edge/validation/gate.py` | `validate_strategy()` returns `verdict`, `score`, `failures`, `metrics`, `triangle`, `profile` |
| Score denominator | `causal_edge/validation/gate.py` | `_count_total()` yields `10` checks base, `12` when `ic_applicable` is true |
...
|---|---|---|
| Score denominator | `10` without IC applicability, `12` with IC applicability | `causal_edge/validation/gate.py`, `.sisyphus/evidence/final-prewave-no-position.txt`, `.sisyphus/evidence/final-prewave-positive.txt` |
...
| `lo_adjusted` | keep | Core retained ratio metric for serial-correlation-aware profiles |
...
| `oos_is` | keep | Retained live gate metric |
...
| `is_sharpe` | keep | Retained intermediate diagnostic for OOS/IS split |
| `oos_sharpe` | keep | Retained intermediate diagnostic for OOS/IS split |
...
| `T12 OOS/IS` | keep | Live documented gate |
...
| `CAPABILITY.md` | Examples describe `10/12`-style applicable-gate denominators and migration notes |
...
|---|---|---|
| Score denominator narrative | mathematical correction | Live contract is `10/12` by applicable-gate scope, not legacy `15` / `20` / `21` wording |
